def max_subarray_sum_left(arr):
    n = len(arr)
    max_sum = [float('-inf')] * (n + 1)  # max_sum[i] is max subarray sum in arr[0..i-1]
    current_sum = 0
    for i in range(n):
...
def max_subarray_sum_right(arr):
    n = len(arr)
    max_sum = [float('-inf')] * (n + 1)  # max_sum[i] is max subarray sum in arr[i+1..n-1]
    current_sum = 0
    for i in range(n - 1, -1, -1):
        current_sum = max(arr[i], current_sum + arr[i])
        max_sum[i] = max(max_sum[i + 1], current_sum)
    max_sum[n] = float('-inf')  # No elements after index n-1
    return max_sum
