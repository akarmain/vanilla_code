...
// Circle это подкласс, а GeometricObject - суперкласс public class Circle
...
Ключевое слово "try" используется для указания блока, в который мы должны поместить код исключения. Это означает, что мы не можем

```java
try
```

использовать блок try в одиночку. За блоком try должен следовать либо catch, либо finally. Блок "catch" используется для обработки исключения. Ему должен catch предшествовать блок try, что означает, что мы не можем использовать блок catch отдельно. За ним может следовать блок finally позже. Блок "finally" используется для выполнения необходимого кода finally программы. Он выполняется независимо от того, обработано исключение или нет. throw Ключевое слово "throw" используется для создания исключения. Ключевое слово "throws" используется для объявления исключений. Оно указывает, что в методе может возникнуть исключение. Оно не

```java
throws
```

выбрасывает исключение. Оно всегда используется с сигнатурой метода.
...
 Три поля данных типа double с именами side1, side2 и side3 и с заданными по умолчанию значениями, равными 1.0, для обозначения трех сторон треугольника.  Безаргументный конструктор, который создает треугольник с заданными по умолчанию значениями.  Конструктор, который создает треугольник с side1, side2 и side3.  Getter-методы для всех трех полей данных.  Метод с именем getArea(), который возвращает площадь этого треугольника.  Метод с именем getPerimeter(), который возвращает периметр этого треугольника.  Метод с именем toString(), который возвращает строковое описание треугольника.

Формулы для вычисления площади треугольника:

![](images/img-005-001.png)
...
```java
return "Треугольник: сторона1 = " + side1 + " сторона2 = " +
```
...
java /** Создает треугольник с указанными сторонами */
...
У одного класса есть некое отношение с другим классом, LinkageError но последний класс несовместимо изменился после компиляции первого класса.

## JVM неисправна или исчерпала ресурсы, необходимые

VirtualMachineError для продолжения работы.
...
## Попытка использовать несуществующий класс. Это исключение может произойти, например, при

ClassNotFoundException

## попытке запуска несуществующего класса с

помощью команды java.

## Связаны с операциями ввода/вывода, такими как

IOException недопустимые входные данные, считывание после конца файла и открытие несуществующего файла.
...
## Деление целого числа на ноль. Арифметические

ArithmeticException операции над числами с плавающей точкой не приводят к исключениям.

## Попытка доступа к объекту с помощью нулевой

NullPointerException переменной ссылочного типа. IndexOutOfBoundsException Индекс массива находится вне его границ.

## Методу передается недопустимый или не

IllegalArgumentException соответствующий ему аргумент.
